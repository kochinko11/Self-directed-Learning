...
class PyMon:
    ...
    def get_min_max_dividend_to_treasury(self, code):
        ...
        treasury_3year={}
        treasury_3year=webreader.get_3year_treasury()
        ...
        for year in range(cur_year - 3, cur_year):
            #print(year)
            #print(previous_dividend_yield[year])
            year2 = str(year)
            #print(treasury_3year[year2])
            if year in previous_dividend_yield.keys() and year2 in treasury_3year.keys():
                dividend_value = previous_dividend_yield[year].replace(',', '')  # Remove commas
                ratio = float(dividend_value) / float(treasury_3year[year2])
                #print(ratio)
